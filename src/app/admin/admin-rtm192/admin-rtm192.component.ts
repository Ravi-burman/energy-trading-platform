...
@Component({
  selector: 'app-admin-rtm192',
  templateUrl: './admin-rtm192.component.html',
  styleUrls: ['./admin-rtm192.component.scss']
})
export class AdminRtm192Component {
  ...
  metricsOptions: any = [ { name: 'RTM', value: 'RTM' },{ name: 'RTM Comparision', value: 'RTMComparision' }, { name: 'All', value: 'All' }  ];
  ...
  ngOnInit() {
    ...
    //this.getRtm192ForeCastVsActualData(this.selectedOption.value,  this.selectRange.value);
  ...
  getRtm192ForeCastVsActualData(view_type, timeframe) {
    this.getDataLoading = true;
    var FromDate = this.datepipe.transform(this.dateRange.value['FromDate'], "dd-MMM-yyyy")?.toString();
    var ToDate = this.datepipe.transform(this.dateRange.value['ToDate'], "dd-MMM-yyyy")?.toString();
    let params: any = {}
    ...
    this.commonService.getRtm192ForecastVsActual({
      params
    }).subscribe((res) => {
      if (res.status_code === 200) {
        this.rowSpan += 1
        ...
        this.getDataLoading = false;
        this.getGraphRtm192ForeCastData();
      }
      else if (res.status_code === 503) {
        if (view_type === 'graph_and_metrics') {
          this.foreCastGraphData.foreCastData = [];
  ...
  onMetricOptionChange() {
    ...
    else if (this.selectedMetric.value === 'RTMComparision') {
      this.getRtm192ForeCastVsActualData(this.selectedOption.value, this.selectRange.value);
      this.getDailyMetricsData(this.selectedOption.value, this.selectRange.value);
      this.foreCastGraphData.foreCastData = [];
      this.approvedForecast = [];
